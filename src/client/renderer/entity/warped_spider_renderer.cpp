...
// Geometry, pose math, and animation below are an exact copy of
// spider_renderer.cpp -- see that file's comment on why this is
// duplicated rather than shared. If a real spider model/animation fix
// ever needs to apply to both, update both files; there is no shared
// source of truth to edit once for both today.
...
WarpedSpiderRenderer::WarpedSpiderRenderer() : haveTex(false) {
    shadowRadius = 0.7f; shadowStrength = 1.0f;
}

void WarpedSpiderRenderer::render(Entity* e, float x, float y, float z, float rot, float a) {
    // Only line that differs from SpiderRenderer::render: the texture path.
    if (!haveTex) { haveTex = textureLoad16("data/images/mob/spider_warped.png", &tex, GU_PSM_5551); if (!haveTex) return; }
    build();
    Mob* mob = (Mob*)e;
