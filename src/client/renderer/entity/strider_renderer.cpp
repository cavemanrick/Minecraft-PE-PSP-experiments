...
void StriderRenderer::render(Entity* e, float x, float y, float z, float rot, float a) {
    if (!haveTex) {
        ...
        // Keep the mob visible on builds whose data pack has not gained the
        // strider texture yet. The geometry is still distinct, and the
        // dedicated texture path can be dropped in without code changes.
        if (!haveTex)
            haveTex = textureLoad16("data/images/mob/pig.png", &tex, GU_PSM_5551);
        if (!haveTex) return;
    }
    build();
    Mob* mob = (Mob*)e;
    ...
    mobRenderParts(mob, parts, S_COUNT, &tex, x, y, z, m.bodyRot, a,
