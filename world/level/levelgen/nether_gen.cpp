...
static void placeFossilSkeleton(World* w, Random& random, int x, int y, int z) {
    ...
    // Cheap headroom probe before the full 24-cell footprint check. The
    // template arcs up to dy=2 above the anchor (see kFossilTemplate), but
    // the only clearance verified before this point is the single cell at
    // y+1 -- nothing had confirmed the 2 full blocks of vertical space the
    // spine's peak and its ribs actually need. Nether Wastes/Soul Sand
    // Valley ceilings are undulating cave terrain, not a flat plain, so
    // most anchor points that pass the y+1 check alone still have a low
    // ceiling 1-2 blocks up and would fail fossilSiteClear anyway -- this
    // just rejects them before paying for the full footprint scan.
    // Mirrors how placeBasaltSpire already probes with netherHeadroom()
    // before attempting its own structure.
    if (netherHeadroom(w, x, y + 1, z, 4) < 3) return;
...
static void decorateSoulSandValley(World* w, Random& random, int xo, int zo) {
    ...
    // soul sand/soil surface it checks for already exists. A few tries per
    // chunk, same tries-based idiom as the spire loop above, rather than a
    // single roll -- this is an 11x5x3 footprint that needs genuinely
    // clear soul sand AND at least 3 blocks of headroom under an
    // undulating cave ceiling, so most individual tries still fail
    // fossilSiteClear/the headroom probe; the point of trying a handful of
    // candidate columns instead of one is to give the chunk more than a
    // single coin-flip at finding one of the rarer spots that actually fit.
    // A prior single-roll-only version effectively never found a fitting
    // site in practice, since the one sampled column plus a single y+1
    // clearance check gave the 24-cell, dy-up-to-2 template almost no room
    // to actually pass fossilSiteClear.
